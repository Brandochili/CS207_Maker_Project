 ...
 *
 *The framework for this code was written by_________________________.
 ...
 *The various cool animations including (twinkle, bouncy balls, meteor,_________) were taken from 
 ...
 *Example: (255, 0, 0) yeilds green as the NeoPixels are set up as GRB. 255 is the maximum brightness of that colour.
 *Example: #F156GH is a colour code. Green = 56, Red = F1, Blue = GH so the code would look like (0x56, 0xF1, 0xGH).
 *For picking your own colours, simply google html colour code finder, pick your colour and place the code into the code.
*/
...
void setup()
{
  ...
  randomSeed(analogRead(1));// set up for the reandom function which picks random colours
}
...
//section below maps the signal from the microphone on to 12 options for LED effects
  ...
  int sound = (volts * 100); // multiply volts by 100 to make all values easier to work with modulus
  sound = (sound%11); // modulus divide to make sure that all volumes play nice on 0 to 11

  int soundLevel = map(sound, 0, 10, 0, 11);
  Serial.print("The volt level is  "); // for testing he values coming in
  Serial.println(volts);//for debugging
  Serial.println(sound);// added to see what 'if' statement should be running
...
// combined with randomised animated patterns to keep it interesting
  ...
  if(soundLevel == 2) //open if sound = 2. Take sound samples in to assign each section a certain amount od LEDs
    {
    // set 3 random variables to set one ranom colour for entire animation  
  ...
  if(soundLevel == 5) //open if sound = 5.
    {
     randomA = random(0, 255);
     randomB = random(0, 255);
     randomC = random(0, 255);
  ...
  if(soundLevel == 6)
    ...
    for (int i = 0; i<30;i++)
    {
    strip.setPixelColor((i+30), strip.Color(200,140,255));
    strip.setPixelColor((29-i), strip.Color(200,140,255));
    strip.show();
    delay(sht);
    }
    */
...
//Below are the functions for the various color and animation functions
